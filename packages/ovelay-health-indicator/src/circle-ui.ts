...
export class CircleUI {
  ...
  private showing?: CircleColor;
  private fadeoutTimeout?: NodeJS.Timeout = undefined;
  ...
  public flash(color: CircleColor) {
    if (!this.showing) {
  ...
  private show(color: CircleColor) {
  ...
  private scheduleFadeout(color: CircleColor) {
    if (this.fadeoutTimeout) {
      this.unscheduleFadeout();
    }

    this.fadeoutTimeout = setTimeout(() => this.hide(), FADEOUT_WAIT_TIME[color.toUpperCase()]);
  }
