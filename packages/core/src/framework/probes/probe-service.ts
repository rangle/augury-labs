...
export class ProbeService {
  ...
  beforeNgBootstrapHook(preBootstrapTargets: any) {
    this.probes.forEach(
      probe =>
        probe.beforeNgBootstrap 
        && probe.beforeNgBootstrap(preBootstrapTargets)
    )
  }
  ...
  afterNgBootstrapHook(ngModuleRef: any) {
    this.probes.forEach(
      probe => 
        probe.afterNgBootstrap 
        && probe.afterNgBootstrap(ngModuleRef)
    )
  }

  // @todo: typing
  private instantiateProbes(ProbeConstructors: Array<new (constructorParams: any) => Probe>): Probe[] {
    return ProbeConstructors.map(
      ProbeConstructor => new ProbeConstructor(
