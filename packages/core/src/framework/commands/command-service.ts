...
export class CommandService {
  ...
  run(request: CommandRequest<any>): CommandResult {
    ...
    const { reactionResults } = this.dispatcher
      .dispatchImmediatelyAndReturn(commandEvent)
  ...
  private callableAPIConstructorFrom(commands: Array<Command<any, any>>): CallableAPIConstructor {
    ...
    return source => commands.reduce(
      (callableAPI, command) => merge(callableAPI, { 
        [methodName(command)]: (args?) => this.run({
          name:command.name,
          args,
          source
        })  
